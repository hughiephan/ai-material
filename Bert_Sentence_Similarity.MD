...
`fugashi` is a Python library used for Japanese morphological analysis, commonly known as tokenization or word segmentation. It is often used in natural language processing (NLP) tasks involving Japanese text, such as part-of-speech tagging, text processing, and language analysis. `ipadic` is a well-known morphological dictionary for Japanese text. It is used by MeCab, the underlying library on which `fugashi` is built. `Transformers` library is used to load the pre-trained BERT model and tokenizer for the Japanese Language (You can also use it for English or Vietnamese Bert). Then we use `scipy.spatial` to calculate the cosine distances between vectors and find related sentences. Cosine similarity is a metric used to determine how similar the documents (other similarity measuring techniques are Euclidean distance or Manhattan distance)
