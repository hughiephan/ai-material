...
## Solution
Solution Criteria: (Seems a bit different with the solution criteria in page 52/62 of Tutorial: An Introduction to Hierarchical Task Network (HTN) Planning))

A bit similar to Executable Task Network
...
## Task Network Solution

![image](https://github.com/hughiephan/DPL/assets/16631121/c6e95aaa-77ce-41f0-9cb4-019d9b138629)
...
(Page 78 should helps with facts, goal description, and other examples)
...
Theorem: $PLAN-EXISTENCE_{HTN}$ is undeciable.

We can prove this by transform our HTN Formalization into two CFG. Then because of "whether the languages of two CFGs have a non-empty intersection" is undecidable [7] then our HTN Formalization will also be undeciable.


![image](https://github.com/hughiephan/DPL/assets/16631121/12784ef2-77c8-4e67-a168-e28909eaeff6)

Two Formal Grammar G and G' was processed in parallel. Each Formal Grammar each has (non-terminal symbols, terminal symbols, production rules, start symbol)
...
$G_1, G_2$, two CFG in Chomsky normal form

Production Rule $R$ and $\acute{R}$ has either form:
...
And $\sum$, set of terminal symbols 

For $i \in {1,2} $, we have:

- $\Gamma$: non-terminal symbols of $G_i$ 

- $S_i \in \Gamma_i$: start symbol of $G_i$

Assume: $\Gamma_1 \cap \Gamma_2  = \emptyset$ 

Define: $\Gamma := \Gamma_1 \cup \Gamma_2$

$L(G_i)$: is language of $G_i$

Assume: $\epsilon \notin L(G_1) \cup L(G_2)$

$R_i$, grammar rules of $G_i$, set $R := R_1 \cup R_2$

Construct $P = (L, C, O, M, c_I, s_I)$ with $c_I \notin \Gamma$ as follows:

- $L := \lbrace (turn_{G1}, turn_{G2}) \rbrace \cup \sum$

- $C := \Gamma \cup \lbrace c_I \rbrace$

- $O := G_i^a | a \in \sum , i \in \lbrace 1,2 \rbrace \cup {F}$

- $M := \lbrace (c_I, tn \rbrace \cup$ 
$X, tn_{X \rightarrow YZ} | X \rightarrow Y Z \in R \rbrace \cup$
$X, tn_{X \rightarrow a} | X \rightarrow a \in R_i, i \in \lbrace 1,2 \rbrace$
